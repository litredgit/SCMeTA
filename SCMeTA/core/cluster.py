...
class Process:
    ...
    def load(
        ...
        self.data.update(load_data(path, target_attr))
    ...
    def show(self, file_name: str | list[str] | None = None, type="xic", attr="raw", refer_mz: float=760.58, scan: int | None = None, output: str="notebook", tol: float | None = None):
        filelist = self.get_filelist(file_name)
        for name in filelist:
            self.data[name].show(type=type, attr=attr, refer_mz=refer_mz, scan=scan, output=output, tol=tol)
